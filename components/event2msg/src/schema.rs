//! Schema definitions for event2msg component.
...
pub fn input_schema() -> SchemaIr {
    ...
    let config_schema = object_schema(vec![
        ...
        ("destination", destination_schema(), true),
        ("text_template", string_schema(1, 4096), false),
    ]);

    object_schema(vec![
        ("event", event_schema, true),
        ("config", config_schema, true),
    ])
}

/// Returns the output schema for the convert operation.
/// Produces a ChannelMessageEnvelope-like structure.
...
pub fn config_schema() -> SchemaIr {
    object_schema(vec![
        ("default_channel", string_schema(1, 64), true),
        ("default_text_template", string_schema(1, 4096), false),
    ])
}
...
fn destination_schema() -> SchemaIr {
    object_schema(vec![
...
fn destinations_array_schema() -> SchemaIr {
    SchemaIr::Array {
        items: Box::new(destination_schema()),
        min_items: None,
        max_items: None,
    }
}
